    ...
    for (const [mkey, m] of ymonths) {
      ...
      for (const r of m.rides) {
        ...
        el.innerHTML = `
          ...
          <div class="rmain">
            <div class="rtitle"><span class="rtitle-text">${esc(r.title) || "Ride"}</span>${r.location ? `<span class="rtitle-loc">${esc(r.location)}</span>` : ""} ${badge(r.status)} ${r.deleted ? deletedBadge() : ""} ${queueBadge(r.key)}</div>
