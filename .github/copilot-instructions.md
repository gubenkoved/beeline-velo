...
| [src/windspeed.ts](../src/windspeed.ts) | Wind-vs-speed analytics: ride segmentation (along + cross-track wind), regression, speed capping, crosswind colour ramp | `segmentRide()`, `linearRegression()`, `speedCapIndices()`, `crossColor()`, `WindSeg` |
| [src/windchart.ts](../src/windchart.ts) | Wind-vs-speed scatter plot (canvas render; optional per-dot crosswind tint via `ChartOpts.dotColor`) | `drawWindSpeedChart()`, `makeScale()`, `niceTicks()`, `ChartOpts` |
| [src/windspeed-view.ts](../src/windspeed-view.ts) | Wind/Speed view (`#analyticsView`): a confirm-to-run gate (centred card naming how many rides the window will analyse, live as the slider moves), the window-scoped per-ride segment sweep (cached, keyed on the segment-geometry tuning), distance-weighted regression, the scatter + KPI cards + empty/blocked states, the crosswind colour toggle + legend + crosswind band filter, and the end-user segment-tuning knobs (look-ahead / turn / min-length + Reset). Behind a `WindSpeedDeps` seam | `initWindSpeedView()`, `mountWindSpeedView()`, `windSpeedVisibleRides()`, `SEG_TUNE_DEFAULTS`, `WindSpeedDeps` |
